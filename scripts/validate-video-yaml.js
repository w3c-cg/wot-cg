...
function validate(filePath) {
  ...
  // thumbnail is required for recorded meetups; optional/null when there is no video.
  const recorded = 'youtube_url' in data && data.youtube_url !== null;
  if (recorded && !('thumbnail' in data && data.thumbnail !== null && String(data.thumbnail).trim())) {
    errors.push("'thumbnail' is required for recorded meetups (an image file in this folder; use null only if the meetup was not recorded)");
  }
  if ('thumbnail' in data && data.thumbnail !== null && typeof data.thumbnail !== 'string')
    errors.push("'thumbnail' must be a string filename (an image file in this folder), or null if the meetup was not recorded");
  if ('description' in data && typeof data.description !== 'string')
    errors.push("'description' must be a string");
  // Generated by build-youtube-description.js; only sanity-checked here.
